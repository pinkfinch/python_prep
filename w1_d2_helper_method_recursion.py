...
# Instantiate the variables that store state
# return them
# create the helper method for recursion
# find base case inside the helper method
...
def list_pairs(lst):
    ...
    def lists(i):
        nonlocal sub_list
        if i == len(lst) - 1:
            sub_list.append([lst[i],None])
            return
        if i >= len(lst): return
        sub_list.append([lst[i], lst[i+1]])
        lists(i+2)
    lists(0)
    return sub_list
...
def power(base, exponent):
    value = 1
    def find_power(num, i):
        nonlocal value
        if i >= exponent: return
        value = value * base
        find_power(value, i+1)
    find_power(base, 0)
    return value
...
#
def merge(lst1, lst2):
    if lst1 is None or len(lst1) == 0: return lst2
    if lst2 is None or len(lst2) == 0: return lst1
    final_list = []
    i, j = 0,0

    def merge_sort(i, j):
        nonlocal final_list
        if i >= len(lst1) and j >= len(lst2) : return;
        if i >= len(lst1) or lst1[i] > lst2[j]:
            final_list.append(lst2[j])
            return merge_sort(i, j+1)
        if j >= len(lst2) or lst1[i] < lst2[j]:
            final_list.append(lst1[i])
            return merge_sort(i+1, j)
        if lst1[i] == lst2[j]:
            final_list.append(lst2[j])
            final_list.append(lst1(i))
            return merge_sort(i+1,j+1)
    merge_sort(0,0)
    return final_list
